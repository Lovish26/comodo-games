...
export const getDateAfterNumDays = function (numDays) {
  const today = getToday();

  const parsedToday = new Date(today);

  const DateAfter3Days = add(parsedToday, { days: numDays });
  const resultISOString = DateAfter3Days.toISOString();
  return resultISOString;
};

export const formatDate = function (dateStr, formatString) {
